...
let draw_game_over world =
  place_image (text "Game Over" 30 black)
              (width / 2, height / 2)
              (draw world)

(* the position last clicked *)
let last_click : (int * int) option ref = ref None

(* preserve the clicked position *)
(* handle_mouse : world_t -> int -> int -> string -> (world_t, 'a) World.t *)
...
(* receive : world_t -> world_t -> (world_t, int * int) World.t *)
