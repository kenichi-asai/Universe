...
(* make_random_ball : int -> Image.color_t -> ball_t *)
...
let draw_game_over world =
  place_image (text "Game Finished" 30 black)
              (width / 2, height / 2)
              (draw world)
...
(* change_world_on_mouse : world_t -> int -> int -> string ->
                           (world_t, 'a) World.t *)
